...
async function getContacts(searchObj, isJWT) {
  try {
    ...
    // CreatedBy already scopes results to the requesting user, so master key here only
    // bypasses class-level permission checks (which otherwise wrongly deny with "Permission
    // denied" instead of an empty result when contracts_Contactbook has never been created yet).
    const contactRes = await mainQuery.find({ useMasterKey: true });
