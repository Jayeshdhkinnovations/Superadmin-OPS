...
// The actual provisioning chain - shared by both createcompany (direct,
// Super Admin picks everything including a generated password) and
// approveRequest (an end user already picked their own password when they
// registered; approval just finally creates their company for real).
export default async function provisionCompany({
  companyName,
  adminName,
  adminEmail,
  adminPassword,
  ...
  try {
    ...
    await writeSystemLog({
      level: 'info',
      route: 'functions/provisioncompany',
    ...
    return {
      ...
      // lookup" routing, already built via loginuser's tenant_redirect) -
      // this is the one shared frontend URL, not a company-specific deep link.
